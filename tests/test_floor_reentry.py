...
class FloorReentryTests(unittest.TestCase):
    def setup_stairs(self,source=1,target=2,radius=26,parent=None):
        ...
        self.stair=DraftItem("double_stairs",200,50,120,350,stair_from=source,
            stair_to=target,stair_direction=direction,stair_right_enabled=False)
        self.other=replace(self.stair,id="destination-stair",stair_from=target,stair_enabled=False,
            stair_right_enabled=True,stair_right_direction="down" if direction=="up" else "up",stair_right_to=source,stair_to=None)
    ...
    def test_rotated_mirrored_nonuniform_building_exit_is_in_world_units(self):
        ...
        self.visit((292,69),(292,74),(292,400));self.assertEqual(self.nav.state.floor,1)
